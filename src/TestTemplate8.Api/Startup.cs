...
using TestTemplate8.Api.Helpers;
using TestTemplate8.Api.Middlewares;
using TestTemplate8.Application;
using TestTemplate8.Core;
using TestTemplate8.Data;

namespace TestTemplate8.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddDbContext<TestTemplate8DbContext>(options =>
            {
                // var connString = new SqlConnectionStringBuilder(_configuration.GetConnectionString("TestTemplate8DbConnection") ?? string.Empty)
                // {
                //     UserID = _configuration["DB_USER"] ?? string.Empty,
                //     Password = _configuration["DB_PASSWORD"] ?? string.Empty,
                // };
                // var connString = new SqlConnectionStringBuilder("Server=tcp:wedevtesttemplate8sql1.database.windows.net,1433;Initial Catalog=wedevtesttemplate8sql1db1;Persist Security Info=False;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;")
                // {
            ...
            if (!string.IsNullOrEmpty(_configuration["APPLICATIONINSIGHTS_CONNECTION_STRING"]))
            {
                services
                    ...
                    //            ResourceBuilder
                    //                .CreateDefault()
                    //                .AddService(serviceName: "TestTemplate8"))
                    //        .AddAspNetCoreInstrumentation()
            ...
            services.AddSwaggerGen(setupAction =>
            {
                ...
                setupAction.SwaggerDoc(
                    "TestTemplate8OpenAPISpecification",
                    new OpenApiInfo
                    {
                        Title = "TestTemplate8 API",
                        Version = "v1",
                        Description = "This API allows access to TestTemplate8.",
                ...
                setupAction.IncludeXmlComments(Path.Combine(AppContext.BaseDirectory, "TestTemplate8.Api.xml"));
            });
            ...
            services.AddCors(o => o.AddPolicy("TestTemplate8Client", builder =>
            {
            ...
            services.AddMassTransit(x =>
            {
                ...
                x.AddEntityFrameworkOutbox<TestTemplate8DbContext>(o =>
                {
            ...
            services.AddTestTemplate8ApplicationHandlers();
            ...
            services
                .AddHealthChecks()
                .AddDbContextCheck<TestTemplate8DbContext>();
        }
        ...
        public void Configure(IApplicationBuilder app)
        {
            ...
            //    app.UseHsts();
            // }
            app.UseCors("TestTemplate8Client");
            ...
            app.UseSwaggerUI(c =>
            {
                c.SwaggerEndpoint("/swagger/TestTemplate8OpenAPISpecification/swagger.json", "TestTemplate8 API");
